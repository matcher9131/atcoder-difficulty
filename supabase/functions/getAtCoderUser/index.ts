...
Deno.serve(async (request) => {
  ...
  const rating = parseInt(
    [...parser.querySelectorAll("tr")].find((tr) =>
      tr.querySelector("th")?.textContent == "Rating"
    )?.querySelector("td span")?.textContent ?? "",
  );
  const numContests = parseInt(
    [...parser.querySelectorAll("tr")].find((tr) =>
      tr.querySelector("th")?.textContent == "Rated Matches "
